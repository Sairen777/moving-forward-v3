...
The "generated" character is carried honestly: a `#`-comment provenance line near the masthead and a footer build stamp both print the field's real parameters (grid size, color count, date) emitted by the generator. The field data itself is a shared generated asset (`field-data.js`) rendered by a small canvas runtime (`field-renderer.js`), so multiple pages keep one source of truth instead of duplicating a heavy inline printout. Structure reads as program output: routes in the nav (`/writing`), dotted plotter rules instead of solid borders, a translucent paper wash so the field keeps breathing in the margins. Density stays low; type is one weight, color is soft, motion is ambient.
...
- Monospace chrome plus a body-only reading serif for long-form articles; hierarchy from size and a 400/500 split, never bold. Modest masthead.
...
**The Generated-Provenance Rule.** Provenance and build stamps print the shared field payload's real parameters (source name, grid size, color count, generated date), never hardcoded or invented. The "generated" feel must be honest.
...
- **Do** edit `tools/page_template.html`, `tools/post_template.html`, or `field-renderer.js`, then rebuild with the generator; never hand-edit generated `index.html`, `post.html`, or `field-data.js`.
