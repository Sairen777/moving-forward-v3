...
BACKGROUND_DIR = ROOT_DIR / "src" / "backgrounds" / "quiet-field"
...
CHAR_ASPECT = 0.601923590766059

FONT = 'ui-monospace,"SF Mono",SFMono-Regular,Menlo,Consolas,"Liberation Mono",monospace'
CELL_ASPECT = CHAR_ASPECT  # matched to renderer
TILE_REPEATS = 3
START_DELAY_MS = 2000
FRAME_MS = 80
MOTION_RAMP_MS = 2500

# Motion constants for the quiet-field background.
MAX_BEND = 2.2
ARC_DROP = 0.07
TIP_POWER = 1.45
HEAD_LAG = 0.22
...
GUST_AMPLITUDE = 0.12
GUST_FREQUENCY = 0.18
GUST_NESTED_AMPLITUDE = 0.35
GUST_NESTED_FREQUENCY = 0.07
WAVES = [
    [0.52, 0.35, 1.5, 0],
    [0.25, 0.65, 3.4, 1],
    [0.1, 1.1, 5.5, 0.5],
...
def _write_background_modules(payload):
    profile_path = BACKGROUND_DIR / "profile.ts"
    bg_css_path = BACKGROUND_DIR / "background.module.css"
    ...
    profile = (
        f"/* Generated by tools/asciify.py field; do not edit by hand. */\n"
        f'import type {{ AsciiBackgroundProfile }} from "../types";\n\n'
        f"export const quietFieldBackground = {{\n"
        f'  id: "quiet-field",\n'
        f"  meta: {meta_json},\n"
        f'  data: {json.dumps(payload["data"])},\n'
        f"  render: {{\n"
        f'    font: {json.dumps(FONT)},\n'
        f"    cellAspect: {repr(CELL_ASPECT)},\n"
        f"    tileRepeats: {TILE_REPEATS},\n"
        f"  }},\n"
        f"  timing: {{\n"
        f"    startDelayMs: {START_DELAY_MS},\n"
        f"    frameMs: {FRAME_MS},\n"
        f"    motionRampMs: {MOTION_RAMP_MS},\n"
        f"  }},\n"
        f"  motion: {{\n"
        f"    maxBend: {MAX_BEND},\n"
        f"    arcDrop: {ARC_DROP},\n"
        f"    tipPower: {TIP_POWER},\n"
        f"    headLag: {HEAD_LAG},\n"
        f"    gustBase: {GUST_BASE},\n"
        f"    gustAmplitude: {GUST_AMPLITUDE},\n"
        f"    gustFrequency: {GUST_FREQUENCY},\n"
        f"    gustNestedAmplitude: {GUST_NESTED_AMPLITUDE},\n"
        f"    gustNestedFrequency: {GUST_NESTED_FREQUENCY},\n"
        f"    waves: {json.dumps(WAVES)},\n"
        f"  }},\n"
        f"}} as const satisfies AsciiBackgroundProfile;\n"
    )
    ...
    bg_css = (
        ...
        f"  user-select: none;\n"
        f"  background: var(--paper);\n"
        f"  font-family: {font_family};\n"
        f"  font-size: calc(100vh / {rows});\n"
        f"  line-height: 1;\n"
        f"}}\n\n"
        f".row {{\n"
        f"  position: absolute;\n"
    ...
    bg_css_path.write_text(bg_css, encoding="utf-8")
    print(
        f"wrote {profile_path.relative_to(ROOT_DIR)} ({len(profile)} bytes) + "
        f"{bg_css_path.relative_to(ROOT_DIR)} ({len(bg_css)} bytes): "
        f"{payload['cols']}x{rows} field, {len(palette)} colors"
    )


def build_field(cols=120):
    _write_background_modules(_field_payload(cols))


def _usage():
    print("usage: tools/asciify.py [field|preview] [cols]")
    print("  field   write src/backgrounds/quiet-field/ (profile.ts + background.module.css), default 120 source columns")
